...
```{r}
mingo_working <- mingo %>% 
  mutate(state_distribution= if_else (dosage_unit> 10000, "large shipment", if_else (dosage_unit > 5000, "medium shipment" , "small shipment")))
...
mingo %>%
  ...
  summarise(shipments = n(),
            ...
            avg_pills_per_shipment = mean(dosage_unit),
            min_pills_in_shipment = min(dosage_unit),
            max_pills_in_shipment = max(dosage_unit), median_pills_in_shipment = median(dosage_unit)
            ) %>%
  arrange(desc(max_pills_in_shipment))
```
...
#the most signficant combo is Strosnider
...
```{r}
mingo %>%
  group_by(reporter_name, reporter_state, reporter_city, reporter_address1, reporter_dea_no) %>%
  summarise(shipments = n(),
            total_pills = sum(dosage_unit)
            ) %>%
...
If I wanted to clean the data so that all three Cardinal Health facilities have the same name, I would create a column then use the filter function and type buyer_name== "Cardinal Health."  
